...
Mission_Time = 2464968.0 + TOF_days    # 

Mission_Time_Data_Message = f"""
{'-'*42}
| Mission TOF: {TOF_days} Days
|
| Mission Start Time: {Julian_D} Days
{'-'*42}
...
#############################################################
# Set plot                                                  #
fig_opt = plt.figure('Optimized Parking Radius')            #
# Set axis                                                  #
axopt = plt.axes()                                          #
# Set Window Size                                           #
fig_opt.set_size_inches(5,5)                                #
axopt.set_title("Saturn Parking Radius vs. Arrival Delta-V")#
axopt.set_xlabel('Parking Radius (km)')                     #
axopt.set_ylabel(' Delta-V from Earth to Saturn (km/s)')    # 
# Plot Data                                      ############
Del_line = Delta_V_To_Sat_opt[0]                            #
axopt.plot(R2_park_opt,Del_line)                            #
plt.legend()                                                #
plt.show() ##################################################
#%% Ephemeris
#############
...
| Titan's Semi-Major axis: {a_Titan} (km)
{'-'*42}
{'#'*42}
{'-'*42}
| Hohmann Transfer
